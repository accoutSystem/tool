...
namespace PD.Business
{
    ...
    public class DataTransaction
    {
        ...
        private void PrepareCommand(DbCommand cmd, DbConnection conn, SqlTransaction trans, string cmdText, DbParameter[] cmdParms)
        {
            ...
            if (cmdParms != null)
            {
                ...
                foreach (DbParameter parameter in cmdParms)
                {
                    ...
                    _db.AddInParameter(cmd, parameter.ParameterName, parameter.DbType, parameter.Value);
                }
            }
        }
        ...
        public static DataTransaction Create(string connection, string vDBKey) {

           var db = new Microsoft.Practices.EnterpriseLibrary.Data.GenericDatabase(connection, new MySql.Data.MySqlClient.MySqlClientFactory());

            _tran = new DataTransaction(db);
             
            //_tran.ConnectionStr = connection;

            return _tran;

        }

        private static Dictionary<string, string> connectonMap = new Dictionary<string, string>();

        /// <summary>
        /// 读取数据库默认连接字符串
        /// </summary>
        /// <returns></returns>
        public static string ReadDefaultConnectString(string key)
        {
        ...
        public static void AddDefaultConnectString(string key, string connectString)
        {
            Configuration config = ConfigurationManager.OpenExeConfiguration(ConfigurationUserLevel.None);
            ConfigurationSection dataConfigurationSection = config.Sections["dataConfiguration"];
            string defaultDatabase = (dataConfigurationSection as Microsoft.Practices.EnterpriseLibrary.Data.Configuration.DatabaseSettings).DefaultDatabase;
            ConfigurationSection connectionConfigurationSection = config.Sections["connectionStrings"];
            ConnectionStringsSection connectionStringsSection = connectionConfigurationSection as ConnectionStringsSection;
            connectionStringsSection.ConnectionStrings.Add(new ConnectionStringSettings { Name = key, ConnectionString = connectString });
            config.Save();
        }
        ...
        public void ExecuteStoredProcedure(SqlParamterItem storedProcedure)
        {
            using (var conn = CreateConnection())
            {
                conn.Open();
                ...
                try
                {
                    ...
                    using (var dbCommand = conn.CreateCommand())
                    {
                        ...
                        dbCommand.CommandText = storedProcedure.Sql;
                        storedProcedure.ParamterCollection.ForEach(item =>
                        {
                            ...
                            {
                                _db.AddOutParameter(dbCommand, item.ParameterName, item.DbType,200);
                            }
                        });
                        ...
                        {
                            var dbParamter = dbCommand.Parameters[i] as DbParameter;
                            if (dbParamter == null)
                                continue;
                            if (dbParamter.Direction == ParameterDirection.Output)
                            {
                                var currentParamter = storedProcedure.ParamterCollection.FirstOrDefault(item => item.ParameterName == dbParamter.ParameterName);
                                if (currentParamter != null)
                                {
                                    currentParamter.Value = dbParamter.Value;
                                }
                            }
                        }
        ...
        public DbConnection CreateConnection()
        {
            ...
            {
                CreatedConnection.ConnectionString = ConnectionStr;
            }
